...
if [ $system == 'slurm' ]; then
    srun -n $ntasks -l -W 0 $SCRIPTS_DIR/prepare_data_pwy_source.sh $velocity_dir 2> ./job_info/error_target
elif [ $system == 'pbs' ]; then 
...
do
    ...
    if [ $system == 'slurm' ]; then
        srun -n $ntasks -l -W 0 $SCRIPTS_DIR/Adjoint_pwy_source.sh $velocity_dir $compute_adjoint 2> ./job_info/error_current_$iter
    elif [ $system == 'pbs' ]; then
...
if $VISCOELASTIC ; then
    ...
    if $TROMP_METHOD; then
      ...
      if [ $system == 'slurm' ]; then
          srun -n $ntasks -l -W 0 $SCRIPTS_DIR/Adjoint_pwy_source_attenuation.sh $velocity_dir $compute_adjoint 2> ./job_info/error_current
      elif [ $system == 'pbs' ]; then
    ...
    if $smooth ; then
        ...
        if $MASK_TOPO ; then
        ## topography mask
            mpirun -np $NPROC_SPECFEM ./bin/mask_kernel_topo.exe $kernel_list,$precond_list $WORKING_DIR $DISK_DIR 2> ./job_info/error_mask_kernel_topo
        fi
    ...
    do
        ...
        if [ $system == 'slurm' ]; then
            srun -n $ntasks -l -W 0 $SCRIPTS_DIR/Adjoint_pwy_source.sh $velocity_dir $compute_adjoint 2> ./job_info/error_update_$iter
        elif [ $system == 'pbs' ]; then
