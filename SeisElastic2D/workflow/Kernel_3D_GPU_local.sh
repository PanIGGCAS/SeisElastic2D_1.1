...
bash $SCRIPTS_DIR/prepare_data_local.sh $velocity_dir 2> ./job_info/error_target
...
bash $SCRIPTS_DIR/Adjoint_3D_local.sh $velocity_dir $compute_adjoint 2> ./job_info/error_current
...
then
    ...
    mpirun -np $NPROC_SPECFEM ./bin/xsmooth_sem $sigma_x $sigma_z $kernel_list $DISK_DIR/misfit_kernel/ $DISK_DIR/misfit_kernel/ true 2> ./job_info/error_smooth_kernel
fi
