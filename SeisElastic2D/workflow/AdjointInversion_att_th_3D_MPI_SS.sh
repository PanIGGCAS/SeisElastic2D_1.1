...
do
    ...
    if [ $system == 'slurm' ]; then
        srun -n $ntasks -l -W 0 $SCRIPTS_DIR/Adjoint_3D.sh $velocity_dir $compute_adjoint 2> ./job_info/error_current_$iter
    elif [ $system == 'pbs' ]; then
...
if $VISCOELASTIC ; then
    ...
    # remove attenuation kernels first
      ...
      if [ $system == 'slurm' ]; then
          srun -n $ntasks -l -W 0 $SCRIPTS_DIR/Adjoint_attenuation.sh $velocity_dir $compute_adjoint 2> ./job_info/error_current
      elif [ $system == 'pbs' ]; then
    ...
    do
        ...
        if [ $system == 'slurm' ]; then
            srun -n $ntasks -l -W 0 $SCRIPTS_DIR/Adjoint_3D.sh $velocity_dir $compute_adjoint 2> ./job_info/error_update_$iter
        elif [ $system == 'pbs' ]; then
