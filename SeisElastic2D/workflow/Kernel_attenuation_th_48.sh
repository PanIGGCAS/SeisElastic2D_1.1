...
if [ $system == 'slurm' ]; then
    srun -n 48 -c $NPROC_SPECFEM -l -W 0 $SCRIPTS_DIR/prepare_data.sh $velocity_dir 2> ./job_info/error_target
elif [ $system == 'pbs' ]; then 
...
if [ $system == 'slurm' ]; then
    srun -n 48 -c $NPROC_SPECFEM -l -W 0 $SCRIPTS_DIR/Adjoint.sh $velocity_dir $compute_adjoint 2> ./job_info/error_current
elif [ $system == 'pbs' ]; then
...
if $VISCOELASTIC ; then
    ...
    if [ $system == 'slurm' ]; then
        srun -n 48 -c $NPROC_SPECFEM -l -W 0 $SCRIPTS_DIR/Adjoint_attenuation.sh $velocity_dir $compute_adjoint 2> ./job_info/error_current
    elif [ $system == 'pbs' ]; then
