...
if [ $system == 'slurm' ]; then
    srun -n $ntasks -l -W 0 $SCRIPTS_DIR/Adjoint_3D.sh $velocity_dir $compute_adjoint 2> ./job_info/error_current
elif [ $system == 'pbs' ]; then
...
if $smooth ; then
    ...
    if [ $solver == 'specfem3D' ]; then 
        ...
        cp -r ./OUTPUT_FILES/DATABASES_MPI/* $DISK_DIR/misfit_kernel
    fi
    mpirun -np $NPROC_SPECFEM ./bin/xsmooth_sem $sigma_x $sigma_z $kernel_list $DISK_DIR/misfit_kernel/ $DISK_DIR/misfit_kernel/ $GPU_MODE > ./job_info/error_smooth_kernel
