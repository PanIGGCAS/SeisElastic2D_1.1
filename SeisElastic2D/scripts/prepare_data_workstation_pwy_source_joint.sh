...
do 
    STARTTIME=$(date +%s)
    if  $ExistDATA && [ -d "$DATA_DIR" ]; then
       bash $SCRIPTS_DIR/copy_data_pwy_source_joint.sh $isource $data_tag $data_list $WORKING_DIR_01 $DISK_DIR_01 $DATA_DIR
       bash $SCRIPTS_DIR/copy_data_pwy_source_joint.sh $isource $data_tag $data_list $WORKING_DIR_02 $DISK_DIR_02 $DATA_DIR
    else
       ...
       bash $SCRIPTS_DIR/Forward_specfem2D_pwy_source.sh $isource $NPROC_SPECFEM $data_tag $data_list \
          $velocity_dir $SAVE_FORWARD $WORKING_DIR_02 $DISK_DIR_02 $DATA_DIR $job 2>./job_info/error_Forward_simulation_02
    fi
 if [ $isource -eq 1 ] ; then
     ENDTIME=$(date +%s)
     Ttaken=$(($ENDTIME - $STARTTIME))
