...
do 
    ...
    bash $SCRIPTS_DIR/Forward_specfem2D_pwy_source.sh $isource $NPROC_SPECFEM $data_tag $data_list \
         $velocity_dir $SAVE_FORWARD $WORKING_DIR_02 $DISK_DIR_02 $DATA_DIR $job 2>./job_info/error_Forward_simulation_01
    ...
    bash $SCRIPTS_DIR/adjoint_source.sh $isource $NPROC_SPECFEM $compute_adjoint $data_list \
        $measurement_list_02 $misfit_type_list $WORKING_DIR_02 $DISK_DIR_02 $Wscale $wavelet_path $VISCOELASTIC $measurement_attenuation 2>./job_info/error_adj_source
    ...
    if $compute_adjoint; then
       ...
       bash $SCRIPTS_DIR/Adjoint_${solver}.sh $isource $NPROC_SPECFEM $data_tag \
            $velocity_dir $SAVE_FORWARD $WORKING_DIR_02 $DISK_DIR_02 2>./job_info/error_Adjoint_simulation
    fi
    if [ $isource -eq 1 ] && $compute_adjoint ; then
       ENDTIME=$(date +%s)
       Ttaken=$(($ENDTIME - $STARTTIME))
