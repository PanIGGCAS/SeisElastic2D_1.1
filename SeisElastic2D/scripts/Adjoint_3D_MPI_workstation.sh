...
do 
    ...
    VISCOELASTIC=false
    STARTTIME=$(date +%s)
    bash $SCRIPTS_DIR/adjoint_source_3D.sh $isource $NPROC_SPECFEM $compute_adjoint $data_list \
    ...
    if $compute_adjoint; then
       data_tag='SEM'
       SAVE_FORWARD=false
       bash $SCRIPTS_DIR/Adjoint_${solver}_MPI_ATT.sh $isource $NPROC_SPECFEM $data_tag \
