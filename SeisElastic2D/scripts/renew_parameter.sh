...
if [ ! -z "$MASK_TOPO" ]; then
    sed -e "s#^LOGICAL :: MASK_TOPO=.*#LOGICAL :: MASK_TOPO=.$MASK_TOPO.#g"  $FILE > temp;  mv temp $FILE
fi

if [ ! -z "$TROMP_METHOD" ]; then
    sed -e "s#^LOGICAL :: TROMP_METHOD=.*#LOGICAL :: TROMP_METHOD=.$TROMP_METHOD.#g"  $FILE > temp;  mv temp $FILE
fi
...
if [ ! -z "$ATTENUATION_SCALING" ]; then
    sed -e "s#^LOGICAL :: ATTENUATION_SCALING=.*#LOGICAL :: ATTENUATION_SCALING=.$ATTENUATION_SCALING.#g"  $FILE > temp;  mv temp $FILE
fi

if [ ! -z "$ATTENUATION_ONLY" ]; then
    sed -e "s#^LOGICAL :: ATTENUATION_ONLY=.*#LOGICAL :: ATTENUATION_ONLY=.$ATTENUATION_ONLY.#g"  $FILE > temp;  mv temp $FILE
fi

if [ ! -z "$scaling_factor_1" ]; then
    sed -e "s#^REAL(KIND=CUSTOM_REAL), PARAMETER :: scaling_factor_1=.*#REAL(KIND=CUSTOM_REAL), PARAMETER :: scaling_factor_1=$scaling_factor_1 #g" $FILE > temp;  mv temp $FILE
fi

if [ ! -z "$scaling_factor_2" ]; then
    sed -e "s#^REAL(KIND=CUSTOM_REAL), PARAMETER :: scaling_factor_2=.*#REAL(KIND=CUSTOM_REAL), PARAMETER :: scaling_factor_2=$scaling_factor_2 #g" $FILE > temp;  mv temp $FILE
fi
